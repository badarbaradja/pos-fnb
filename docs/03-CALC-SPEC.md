...
type CalcSettings = {
  ...
  serviceChargeInTaxBase: boolean; // default true
...
### A.2 Urutan langkah (WAJIB berurutan)
...
8.  taxBase              = Σ netAmount_i untuk baris isTaxable
                           + serviceCharge (kalau serviceChargeInTaxBase)
...
10. total                = roundTo(totalBeforeRounding, roundingTo)   [pembulatan NORMAL, bukan ke bawah]
    rounding             = total − totalBeforeRounding
```

Catatan: `discountBase = 0` → semua alokasi 0, jangan bagi nol.

### A.3 Golden test cases

**TC-01 — Kasus standar (wajib lolos persis)**

Input:
```
Line 1: Latte,      qty 2, unitPrice 28.000, modifier 5.000, itemDiscount 0
Line 2: Nasi Goreng, qty 1, unitPrice 35.000, modifier 0,     itemDiscount 5.000
Settings: orderDiscountPercent 10%, maxDiscount 15.000,
          serviceCharge 5%, tax 10%, taxInclusive false, roundingTo 100
```
Expected:
```
grossAmount L1        = 66.000
grossAmount L2        = 35.000
subtotal              = 101.000
itemDiscountTotal     =   5.000
discountBase          =  96.000
orderDiscount         =   9.600
allocated L1          =   6.600
allocated L2          =   3.000
...
```
1 × Kopi Tubruk 15.000, tax 0%, service 0%, roundingTo 100
→ total = 15.000, rounding = 0, taxAmount = 0
```

**TC-03 — Harga sudah termasuk pajak (taxInclusive = true)**
```
1 × Menu 110.000, tax 10%, service 0%, roundingTo 1
→ taxBase = 110.000
→ taxAmount = 110.000 − (110.000 / 1,1) = 10.000
→ total = 110.000  (pelanggan tetap bayar 110.000)
```

**TC-04 — Sisa pembulatan alokasi diserap baris terakhir**
```
3 baris @ 10.000 (total 30.000), orderDiscount 10.000
→ alokasi naif: 3.333,33 × 3 = 9.999,99 ≠ 10.000
→ Expected: 3.333,33 / 3.333,33 / 3.333,34  → Σ = 10.000 PERSIS
Assertion wajib: Σ allocated === orderDiscount
```

**TC-05 — Diskon melebihi tagihan**
```
subtotal 50.000, orderDiscountAmount 80.000
→ orderDiscount di-cap jadi 50.000, netSales = 0, total = 0
→ tidak boleh menghasilkan angka negatif
```

**TC-06 — Item non-taxable dicampur**
```
Line 1: Makanan 100.000, isTaxable true
Line 2: Voucher parkir 5.000, isTaxable false
tax 10%, service 0%
→ taxBase = 100.000 (bukan 105.000)
→ taxAmount = 10.000
→ total = 115.000
```
...
total                >= 0
rounding             berada di rentang (−roundingTo/2, +roundingTo/2]
```
...
```
Biji kopi   18 g   × avgCost 145      , yield 100%, waste 3%  → 2.688,30
Susu UHT   200 ml  × avgCost 18,5     , yield 100%, waste 0%  → 3.700,00
Gula cair   10 ml  × avgCost 12       , yield 100%, waste 0%  →   120,00
Cup + lid    1 pcs × avgCost 1.350    , yield 100%, waste 0%  → 1.350,00
────────────────────────────────────────────────────────────────────────
hpp = 7.858,30
```
Turunan: harga jual 28.000 → foodCostPercent = 28,06%

**TC-09 — Yield di bawah 100%**
```
Ayam fillet: recipeQty 150 g, avgCost 45/g, yield 80%
→ effectiveCost = 45 / 0,8 = 56,25
→ lineCost = 150 × 56,25 = 8.437,50
```
...
**TC-10 — WAC dasar**
```
Stok 1000 g @ 145, masuk 1000 g @ 160
→ newAvgCost = (145.000 + 160.000) / 2000 = 152,50
```
...
```
Beli: kopi 5 kg @ 145.000 = 725.000 ; susu 20 l @ 18.500 = 370.000
Ongkir 50.000
→ alokasi kopi = 50.000 × 725/1095 = 33.105,02 → per gram +6,62
→ alokasi susu = 50.000 × 370/1095 = 16.894,98 → per ml   +0,84
...
**TC-13 — P&L satu hari**
```
grossSales 10.000.000 ; diskon 500.000 ; refund 200.000
→ netSales     = 9.300.000
cogs 2.900.000 → grossProfit = 6.400.000 (68,82%)
labor 1.800.000 + sewa 500.000 + utilitas 300.000 + penyusutan 200.000 = 2.800.000
→ operatingProfit = 3.600.000
pajak 0 → netProfit = 3.600.000 (38,71%)
```
...
**TC-14**
```
openingCash 500.000 ; tunai masuk 3.200.000 ; kembalian 150.000
cashIn 0 ; cashOut 75.000 (beli galon) ; refund tunai 0
→ expectedCash = 3.475.000
countedCash 3.460.000 → cashVariance = −15.000 (kurang)
```
